...
def make_disruption_generator(delivery_points, driver_capacity=3):
    ...
    def generate_disruption(current_time):
        ...
        # Random location near a delivery point
        ...
        # For vehicle breakdowns, generate random affected drivers
        affected_driver_ids = set()
        if disruption_type == DisruptionType.VEHICLE_BREAKDOWN:
            num_affected = random.randint(1, driver_capacity)
            for _ in range(num_affected):
                affected_driver_ids.add(random.randint(1, 10))
...
def main():
    ...
    if args.load:
        ...
        agent.epsilon = 0.1  # Lower exploration rate for pre-trained model
