...
class RLResolver(DisruptionResolver):
    ...
    def __init__(self, model_path, graph, warehouse_location):
        ...
        # Create a dummy environment for action decoding
        env_config = {
            "graph": graph,
    ...
    def should_recalculate(self, state: DeliverySystemState, disruption: Disruption) -> bool:
        ...
        # If the highest action value is significantly better than no action (index 0),
        # then recalculation is worth it
        no_action_value = action_values[0]
        best_action_value = np.max(action_values)

        # Set a threshold for recalculation
        threshold = 2.0  # Expected reward improvement to justify recalculation
    ...
    def on_disruption_detected(self, disruption: Disruption, state: DeliverySystemState) -> list:
        ...
        """
        # Set the disruption as the latest one in the state
